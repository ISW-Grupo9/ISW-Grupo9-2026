...
public class CompraServiceImpl implements CompraService {
    ...
    public CompraServiceImpl(ValidadorFechaService validadorFecha,
    ...
    public CompraResponse crear(CompraRequest request) {
        ...
        List<Visitante> visitantes = request.visitantes().stream()
                .map(dto -> new Visitante(dto.nombre(), dto.edad(), dto.tipoPase()))
                .toList();

        Compra compra = new Compra(request.usuarioId(), request.fechaVisita(), visitantes, request.formaPago(), total);
