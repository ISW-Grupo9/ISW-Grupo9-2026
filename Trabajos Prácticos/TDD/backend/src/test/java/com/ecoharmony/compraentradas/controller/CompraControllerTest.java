...
class CompraControllerTest {
  ...
  void post_retorna_201_con_pago_efectivo() throws Exception {
    CompraResponse response =
        new CompraResponse(
            ...
            FormaPago.EFECTIVO,
            EstadoCompra.PENDIENTE_BOLETERIA,
            null);
  ...
  void get_retorna_404_si_compra_no_existe() throws Exception {
    ...
    mockMvc.perform(get("/api/compras/99")).andExpect(status().isNotFound());
  }
  ...
  void post_confirmar_retorna_200_con_estado_confirmada() throws Exception {
    ...
    mockMvc
        .perform(post("/api/compras/1/confirmar"))
        .andExpect(status().isOk())
  ...
  void post_confirmar_retorna_404_si_compra_no_existe() throws Exception {
    ...
    mockMvc.perform(post("/api/compras/99/confirmar")).andExpect(status().isNotFound());
  }
}
