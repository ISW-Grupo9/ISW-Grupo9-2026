...
describe('CompraPage — flujo completo', () => {
  ...
  it('compra exitosa con efectivo muestra la confirmación', async () => {
    server.use(
      http.post('/api/compras', () =>
        HttpResponse.json({
          id: '1', fechaVisita: FECHA_FUTURA, cantidadEntradas: 1,
          montoTotal: 5000, formaPago: 'EFECTIVO', estado: 'PENDIENTE_BOLETERIA',
        }, { status: 201 })
      )
    )
  ...
  it('compra exitosa con tarjeta llama a redirect con URL de MP', async () => {
    ...
    server.use(
      http.post('/api/compras', () =>
        HttpResponse.json({
          id: '1', fechaVisita: FECHA_FUTURA, cantidadEntradas: 1,
          montoTotal: 5000, formaPago: 'TARJETA', estado: 'PENDIENTE',
  ...
  it('submit sin forma de pago muestra error y no envía la request', async () => {
    ...
    server.use(http.post('/api/compras', () => { fetchSpy(); return HttpResponse.json({}) }))
