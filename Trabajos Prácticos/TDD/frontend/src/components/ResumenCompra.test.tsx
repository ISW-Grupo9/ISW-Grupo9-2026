...
const regular = (n = 1): Visitante[] => Array.from({ length: n }, (_, i) => ({ nombre: `R${i}`, edad: 20, tipoPase: 'REGULAR' as const }))
const vip     = (n = 1): Visitante[] => Array.from({ length: n }, (_, i) => ({ nombre: `V${i}`, edad: 30, tipoPase: 'VIP'     as const }))

describe('ResumenCompra', () => {
  // ─── Ciclo 18.1 ───────────────────────────────────────────────────────
  it('muestra la cantidad de entradas', () => {
    render(<ResumenCompra fechaVisita="2026-06-15" visitantes={[...regular(), ...regular(), ...vip()]} />)
